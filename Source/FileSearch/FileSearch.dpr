...
{
  Алгоритм Boyer-Moore-Horspool для быстрого поиска шаблона в буфере.
  @param PBuf Адрес начала буфера
  @param BufLen Длина буфера
  @param Pattern Массив байтов искомого шаблона
  @return Позицию первой найденной копии шаблона (-1, если не найдено)
}
...
{
  Парсит строку, содержащую маски поиска, разделённые символом "|".
  @param MaskStr Входная строка с масками
  @return Список строк-маск
}
...
{
  Разбирает строку с шаблонами поиска, создавая объекты для каждого шаблона.
  @param PatternStr Строка с шаблонами, разделённая символом "|"
  @return Список объектов-шаблонов
}
...
{
  Выполняет задачу поиска файлов по указанным путям и маске.
...
{
  Реализует задачу поиска двоичных последовательностей в указанном файле.
  @param Params Строка с параметрами задания
  @param ProgressCallback Коллбэк-функция для передачи хода выполнения
  @param UserData Дополнительные данные для коллбэка
  @return True, если задача была успешно выполнена
...
{
  Определяет и выполняет нужную задачу согласно переданному названию.
...
{
  Принудительно останавливает запущенный процесс.
}
