...
uses
  Windows, SysUtils, Classes, ShellAPI, System.JSON, SharedTypes;
...
begin
  ...
  begin
    // Параметры: команда и рабочая папка
...
{
  Осуществляет чтение вывода команды и передает его через ProgressCallback.
  @param ProgressCallback Коллбэк-функция для отправки данных
  @param UserData Дополнительные данные для коллбэка
  @return True, если обработка завершилась успешно
}
...
var
  ...
  {
    Преобразует OEM-кодировку в стандартную ANSI.
    @param OemStr Строка в OEM-коде
    @return Строка в ANSI-коде
  }
  function OemToAnsiString(const OemStr: AnsiString): AnsiString;
  begin
...
begin
  ...
  repeat
    ...
    begin
      Buffer[BytesRead] := #0;
      OemText := AnsiString(Buffer);
      AnsiText := OemToAnsiString(OemText);
      LineBuffer := LineBuffer + string(AnsiText);
    ...
    begin
      if Assigned(ProgressCallback) then
      begin
        // Процесс завершился — отправляем остаток буфера
        LineBuffer := Trim(LineBuffer);
        if LineBuffer <> '' then
          ProgressCallback(50, PAnsiChar(AnsiString('Выполняется...')), PAnsiChar(AnsiString(LineBuffer)), UserData);
        if FTerminated then
